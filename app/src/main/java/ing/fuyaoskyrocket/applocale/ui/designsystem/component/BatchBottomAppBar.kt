...
data class AppBatchSelectionActions(val count: Int, val onClose: () -> Unit, val onSelectAll: () -> Unit, val onClear: () -> Unit)
...
fun BatchBottomAppBar(
    ...
    selectionActions: AppBatchSelectionActions? = null,
) {
    if (AppUiTheme.policy.controls == AppControlFamily.Eclair) {
        ing.fuyaoskyrocket.applocale.ui.designsystem.eclair.EclairBatchPanel(
            requireNotNull(selectionActions), hasSelection, isApplying, onClick, modifier, interactive,
        )
        return
    }
    ...
    if (AppUiTheme.policy.controls == AppControlFamily.Material2) {
        androidx.compose.material.Surface(modifier = presentationModifier,
            shape = androidx.compose.ui.graphics.RectangleShape,
            color = AppUiTheme.palette.surface, elevation = AppUiTheme.elevation.toolbar) {
            BatchBarContent(hasSelection, isApplying, presentationProgress, interactive, onClick,
                Modifier.fillMaxWidth().navigationBarsPadding().padding(horizontal = AppSpacing.lg, vertical = AppSpacing.sm))
        }
