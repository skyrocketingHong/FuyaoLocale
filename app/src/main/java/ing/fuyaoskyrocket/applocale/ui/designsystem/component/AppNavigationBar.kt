...
import androidx.compose.material.BottomNavigationItem as Material2NavigationItem
...
private fun appNavigationItems(
    ...
    AppNavigationItem(
        ...
        labelRes = R.string.settings_category,
...
private fun AppNavigationItem.icon(selected: Boolean): ImageVector =
    if (AppUiTheme.policy.controls == AppControlFamily.Miuix) {
        when (destination) {
            ...
            AppNavigationDestination.Configurations -> MiuixIcons.Regular.Folder
            AppNavigationDestination.Settings -> MiuixIcons.Regular.Settings
        ...
        when (destination) {
            ...
            AppNavigationDestination.Settings ->
                if (selected) Icons.Filled.FilledSettings else Icons.Outlined.OutlinedSettings
...
private fun NavigationItemLabel(labelRes: Int) {
    val labelColor = if (AppUiTheme.policy.controls == AppControlFamily.Material2)
        androidx.compose.material.LocalContentColor.current.copy(alpha = androidx.compose.material.LocalContentAlpha.current)
        else LocalContentColor.current
    BasicText(
        text = stringResource(labelRes),
        style = AppUiTheme.textStyles.label.copy(color = labelColor),
        maxLines = 1,
...
fun AppNavigationBar(
    ...
    if (AppUiTheme.policy.navigation != ing.fuyaoskyrocket.applocale.ui.designsystem.AppNavigationPresentation.AdaptiveBottomOrRail) return
    ...
    if (AppUiTheme.policy.controls == AppControlFamily.Material2) {
        androidx.compose.material.BottomNavigation(
            windowInsets = WindowInsets.navigationBars.only(WindowInsetsSides.Bottom).union(WindowInsets.safeDrawing.only(WindowInsetsSides.Horizontal)),
            modifier = modifier,
            backgroundColor = AppUiTheme.palette.surface,
            ...
            items.forEach { item ->
                ...
                Material2NavigationItem(
                    selected = selected, onClick = item.onClick, enabled = effectiveEnabled,
                    icon = { androidx.compose.material.Icon(item.icon(selected), null) },
                    label = { NavigationItemLabel(item.labelRes) }, alwaysShowLabel = true,
                    modifier = Modifier.tabTouchTapObserver(item.destination, { effectiveEnabled }) {
                        handleTabTap(it as AppNavigationDestination)
                    }.semantics {
                        if (selected) customActions = listOf(CustomAccessibilityAction(scrollToTopActionLabel) {
                            latestOnTabDoubleTap(currentDestination); true
                        })
                    },
                )
            }
    ...
    if (AppUiTheme.style == AppThemeStyle.MATERIAL3_EXPRESSIVE) {
        androidx.compose.material3.ShortNavigationBar(
            modifier = modifier.dockBarBackdrop(),
            containerColor = if (moreBlurActive) Color.Transparent else MaterialTheme.colorScheme.surfaceContainer,
        ) {
            items.forEach { item ->
                val selected = item.destination == currentDestination
                androidx.compose.material3.ShortNavigationBarItem(
                    ...
                    modifier = Modifier.tabTouchTapObserver(
                        ...
                        enabled = { effectiveEnabled },
                        onTap = { handleTabTap(it as AppNavigationDestination) },
                    ).semantics {
                        if (selected) customActions = listOf(CustomAccessibilityAction(scrollToTopActionLabel) {
...
fun AppNavigationRail(
    ...
    val latestOnDoubleTap by rememberUpdatedState(onTabDoubleTap)
    val scrollToTopActionLabel = stringResource(R.string.scroll_to_top)
    LaunchedEffect(currentDestination) { detector.clear() }
    fun onTap(target: Any) {
        if (target == currentDestination && detector.recordTap(target)) {
            latestOnDoubleTap(currentDestination)
        }
    }
    @Composable
    fun itemModifier(item: AppNavigationItem): Modifier = Modifier.tabTouchTapObserver(
        target = item.destination,
        enabled = { true },
        onTap = ::onTap,
    ).semantics {
        if (item.destination == currentDestination) {
            customActions = listOf(CustomAccessibilityAction(scrollToTopActionLabel) {
                latestOnDoubleTap(currentDestination)
                true
            })
        }
    ...
    if (AppUiTheme.policy.controls == AppControlFamily.Material2) {
        androidx.compose.material.NavigationRail(
            windowInsets = WindowInsets.systemBars.only(WindowInsetsSides.Vertical).union(WindowInsets.safeDrawing.only(WindowInsetsSides.Horizontal)),
            modifier = modifier.fillMaxHeight(), backgroundColor = AppUiTheme.palette.surface,
            contentColor = AppUiTheme.palette.foreground, elevation = 0.dp,
        ) {
            Spacer(Modifier.height(AppSpacing.lg))
            items.forEach { item ->
                val selected = item.destination == currentDestination
                androidx.compose.material.NavigationRailItem(
                    selected = selected, onClick = item.onClick,
                    icon = { androidx.compose.material.Icon(item.icon(selected), null) },
                    label = { NavigationItemLabel(item.labelRes) }, alwaysShowLabel = true,
